...
export default function LibraryPage() {
    ...
    async function loadLibrary() {
      ...
      try {
        const res = await apiClient<{
          success: boolean;
          data: { saved: Prompt[]; favorites: Prompt[] };
        }>("/library");
  ...
  const currentList = activeTab === "saved" ? savedPrompts : favoritePrompts;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="mb-8 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-2">
            Aceda rapidamente aos prompts guardados e favoritos da sua conta ({user?.name}).
          </p>
        </div>

        <Link href="/explore">
          <Button variant="outline" size="sm">
            <Compass className="w-4 h-4 mr-1.5 text-brand-400" />
      ...
      {/* Tabs */}
      <div className="flex items-center gap-2 border-b border-surface-border pb-4 mb-8">
        <button
        ...
        >
          ...
          <span>Prompts Guardados ({savedPrompts.length})</span>
        </button>
      ...
      {isLoading ? (
        ...
        <div className="glass p-12 rounded-3xl text-center max-w-md mx-auto border border-surface-border my-12">
          ...
          <h3 className="text-base font-bold text-white mb-2">
            Nenhum prompt {activeTab === "saved" ? "guardado ainda" : "marcado como favorito"}
          </h3>
          <p className="text-xs text-slate-400 mb-6">
            Quando encontrar prompts incríveis no catálogo, clique em &ldquo;Salvar&rdquo; ou no ícone de coração para organizar sua coleção pessoal.
          </p>
          <Link href="/explore">
            <Button variant="primary" size="md">
              Explorar Catálogo de Prompts
            </Button>
          </Link>
        </div>
      ) : (
